...
dual-window identifiers, richer operator outcomes, and stricter mount and
runtime-event durability. It intentionally connects no learning engine. See
...
App Server fields are version-specific observations. The telemetry development
snapshot was generated by Codex CLI 0.144.5; routed mode is now pinned to the
separately regenerated and reviewed Codex CLI 0.144.6 contract. The exact
compatibility gate continues to fail closed when the installed version does not
match that snapshot. Telemetry does not bypass the gate.
